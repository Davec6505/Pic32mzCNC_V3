...
**Settings Configured**:
```gcode
$102=6400.0      # Z steps/mm = (200 × 32) / 1mm = 6400
...
**Calculation**:
```
steps_per_mm = (motor_steps_per_rev × microstepping) / pitch
             = (200 × 32) / 1mm
             = 6400 steps/mm
```
...
## 🆕 New Feature: $21 Hard Limits Enable
...
|---------|---------|----------------|----------------|
| $21 | Hard limits enable | ✅ Yes | ❌ No |
| $22 | Homing enable | ❌ No | ✅ Yes |
...
## 🔍 Limit Switch System Verification
...
```
NO Switch ($5 bit = 0):
  Pin LOW (0):  0 ^ 0 = 0 (no alarm) ✅
  Pin HIGH (1): 1 ^ 0 = 1 (alarm - switch closed) ✅

NC Switch ($5 bit = 1):
  Pin LOW (0):  0 ^ 1 = 1 (alarm - switch open/broken) ✅
  Pin HIGH (1): 1 ^ 1 = 0 (no alarm - switch closed) ✅
```
...
bool HOMING_LimitTriggered(void) {
    ...
    // Hardware debouncing (default 25µs)
