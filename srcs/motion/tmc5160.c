...
static const TMC5160_AxisConfig g_default_cfg = {
    ...
    .mres        = TMC5160_MRES_16,  // 16 microsteps (match $102/$112/$122/$132)
    .stealthchop = true,             // StealthChop (quiet) enabled
    .tpwm_thrs   = 0,               // 0 = StealthChop always on
};
...
// PWMCONF: StealthChop defaults from TMC5160 datasheet section 22
...
void TMC5160_ConfigAxis(E_AXIS axis, const TMC5160_AxisConfig* cfg)
{
    ...
    // 2. GCONF: enable StealthChop if requested
    uint32_t gconf = 0;
    if (cfg->stealthchop) {
    ...
    // 3. IHOLD_IRUN: motor current settings
    ...
    // 4. TPOWERDOWN: time before hold current ramp-down (10 = ~160ms)
    TMC5160_WriteRegister(axis, TMC5160_REG_TPOWERDOWN, 10);

    // 5. TPWMTHRS: StealthChop upper threshold (0 = always StealthChop)
    TMC5160_WriteRegister(axis, TMC5160_REG_TPWMTHRS, cfg->tpwm_thrs);

    // 6. CHOPCONF: chopper config + microstep resolution
    uint32_t chopconf = CHOPCONF_BASE | ((uint32_t)(cfg->mres & 0x0F) << 24);
    TMC5160_WriteRegister(axis, TMC5160_REG_CHOPCONF, chopconf);

    // 7. PWMCONF: StealthChop parameters (datasheet defaults)
    if (cfg->stealthchop) {
        TMC5160_WriteRegister(axis, TMC5160_REG_PWMCONF, PWMCONF_DEFAULT);
    }

    DEBUG_PRINT_MOTION("[TMC5160] Axis %d configured: irun=%d ihold=%d mres=%d sc=%d\r\n",
                       (int)axis, cfg->irun, cfg->ihold, cfg->mres, (int)cfg->stealthchop);
}
...
void TMC5160_Initialize(void)
{
    ...
    // Deassert all CS lines before SPI begins
    ...
    // Configure all axes with default settings
    for (E_AXIS axis = AXIS_X; axis < NUM_AXIS; axis++) {
        TMC5160_ConfigAxis(axis, &g_default_cfg);
        CORETIMER_DelayUs(10); // Brief gap between axes
    }

    g_initialized = true;
    UART_Printf("[TMC5160] Initialized (%d axes)\r\n", (int)NUM_AXIS);
}
...
bool TMC5160_Tasks(void)
{
    ...
    for (E_AXIS axis = AXIS_X; axis < NUM_AXIS; axis++) {
        ...
        TMC5160_Status* s = &g_status[axis];
        s->drv_status    = drv;
