...
void STEPPER_LoadSegment(MotionSegment* segment)
{
    ...
    // OC1R/OC1RS are fixed at the START of each period (tick 2 and 4).
    // This ensures the ISR always fires while the timer is still LOW (tick 4),
    // so writing a new PR4 in the ISR is always reachable (PR4 >= 7 > 4).
    // Placing them near the END (period-5 / period-3) caused the timer to
    // be PAST the new, smaller PR4 during acceleration → 65535-tick wrap → slow.
    OCMP1_CompareValueSet(2u);           // OC1R: OC output HIGH at tick 2
    OCMP1_CompareSecondaryValueSet(4u);  // OC1RS: ISR fires at tick 4 (always < PR4 >= 7)

    if (!(OC1CON & _OC1CON_ON_MASK)) { OCMP1_Enable(); }
    TMR4 = 0u;                           // Reset timer: ensures clean period start
...
void STEPPER_ResumeMotion(void)
{
    ...
    if (current_segment != NULL) {
        ...
        if (period < 7u) { period = 7u; }
        TMR4_PeriodSet(period);
        OCMP1_CompareValueSet(2u);           // OC1R fixed early in period
        OCMP1_CompareSecondaryValueSet(4u);  // OC1RS fixed — ISR at tick 4
        OCMP1_Enable();
        TMR4 = 0u;  // Reset timer for clean period start
        TMR4_Start();
        app_data_ref->motionActive = true;
...
void OCP1_ISR(uintptr_t context)
{
    ...
    if (new_tail != app_data_ref->segmentBufferHead) {
        ...
        // OC1R/OC1RS stay fixed at 2/4 — only PR4 changes per segment.
        // Timer is currently at ~4 (OC1RS fire point); new PR4 >= 7 > 4,
        // so the timer always reaches new PR4 without wrapping. No TMR4 reset needed.
        uint16_t period = (uint16_t)current_segment->step_interval;
        if (period < 7u) { period = 7u; }
        PR4   = period;  // Set new step rate
        OC1R  = 2u;      // Fixed: OC output pulse start (tick 2)
        OC1RS = 4u;      // Fixed: ISR fires at tick 4 in every period
    } else {
...
// ============================================================================
// Real-time feedrate accessor
// Reads the hardware step_interval from the currently executing StepSegment
// (updated every segment by the ISR) and converts to mm/min.
// Safe to call from main loop – reads are atomic on MIPS (32-bit aligned loads).
// ============================================================================
float STEPPER_GetCurrentFeedrateMmMin(void)
{
    if (current_segment == NULL) return 0.0f;
    uint32_t interval = current_segment->step_interval;
    if (interval == 0) return 0.0f;
    float steps_per_mm = stepper_pos.steps_per_mm[dominant_axis];
    if (steps_per_mm <= 0.0f) return 0.0f;
    // step_interval is timer ticks between dominant-axis steps
    float steps_per_sec = (float)TMR4_FrequencyGet() / (float)interval;
    return (steps_per_sec / steps_per_mm) * 60.0f;
}
