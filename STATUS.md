...
## ✅ FEATURE IMPLEMENTATION: G28/G30, status A-axis + Bf: + $10 mask (March 2026)

### G28 / G28.1 / G30 / G30.1 — stored machine positions
- `incs/settings/settings.h` — Added `float g28_position[4]` and `float g30_position[4]` to `CNC_Settings`; added `SETTINGS_GetG28Position`, `SETTINGS_SetG28Position`, `SETTINGS_GetG30Position`, `SETTINGS_SetG30Position` prototypes; bumped `SETTINGS_VERSION`: 5→6 (DRV8825), 6→7 (TMC5160)
- `srcs/settings/settings.c` — Added `.g28_position = {0,0,0,0}` / `.g30_position = {0,0,0,0}` zero defaults; added 4 getter/setter implementations using `memcpy` + `SETTINGS_SaveToFlash()`
- `incs/gcode/gcode_parser.h` — Added 4 new `GCODE_EventType` values: `GCODE_EVENT_GOTO_G28`, `GCODE_EVENT_GOTO_G30`, `GCODE_EVENT_SET_G28`, `GCODE_EVENT_SET_G30`
- `srcs/gcode/gcode_parser.c:657` — `parse_command_to_event()`: G28/G28.1/G30/G30.1 parsed before G33 block; subcode via `strchr(cmd, '.')`
- `srcs/gcode/gcode_parser.c:1396` — `$#` handler: prepends `[G28:x,y,z,a]` and `[G30:x,y,z,a]` lines before G54–G59; buffer expanded 400→512
- `srcs/motion/motion_bridge.c:905` — Added `GCODE_EVENT_SET_G28` / `GCODE_EVENT_SET_G30`: drain, read `KINEMATICS_GetCurrentPosition()`, call `SETTINGS_SetG2xPosition()`
- `srcs/motion/motion_bridge.c:905` — Added `GCODE_EVENT_GOTO_G28` / `GCODE_EVENT_GOTO_G30`: resolve start, build end from stored position, rapid via `cc_rapid_fr()` × rapid override, `TRAJECTORY_AddMove()`

### Status report — A-axis coordinates + `Bf:` buffer state
- `incs/motion/motion.h` — Added `uint32_t MOTION_GetTrajectoryFree(void)` declaration
- `srcs/motion/motion_bridge.c:1300` — Added `MOTION_GetTrajectoryFree()` implementation: `TRAJ_QUEUE_SIZE - TRAJECTORY_QueueCount()`
- `srcs/gcode/gcode_parser.c:989` — `service_realtime_byte()` `case '?'`: format string extended to 4-axis `MPos`/`WPos` (added `mpos[AXIS_A]`, `wpos[AXIS_A]`); `char bf_buf[24]` added; gated on `status_report_mask & 0x02` → fills `|Bf:free,rxbytes`; extra `%s` in format for `bf_buf`
